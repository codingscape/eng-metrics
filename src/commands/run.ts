...
export async function runReport(args: RunArgs) {
  ...
  const prs =
    cfg.github.repos.mode === 'allowlist'
      ? prsAll.filter((pr: any) => cfg.github.repos.allowlist.includes(pr.base.repo.name))
      : prsAll;
  ...
  for (const login of uniqueLogins) {
    if (displayNameByLogin[login]) continue;
    const profile = await getUserProfile(gh, login);
    if (profile?.name) displayNameByLogin[login] = profile.name;
  }
